...
class Decode(nn.Module):
    def __init__(
        ...
        self.recon_block = nn.Sequential(
            nn.Linear(
                latent_dimension,
                int(output_size / (pooling_size) ** 3) * hidden_channels * 4,
            )
        )
...
class Pilati_model_5_layer(nn.Module):
    def __init__(
        self,
        input_size: int,
        input_channel: int,
        ...
        self.final_dense = nn.Sequential(
            nn.Linear(hidden_channel * int(256), 20), activation, nn.Linear(20, 1)
        )
    ...
    def __init__(
        ...
        for i in range(len(hidden_neurons)):
            if i == 0:
                ...
                self.decoder.append(
                    nn.Linear(hidden_neurons[-i], hidden_neurons[-1 - i])
                )
