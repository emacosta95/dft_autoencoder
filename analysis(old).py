...
models_name = [
    [
        f"meyer_case_2ndGEN/test_meyer_30_hc_13_ks_2_ps_[40, 40, 40, 40, 40]_hidden_neurons_16_ls_0.5_alpha_0.001_vb"
    ]
    for v in ls
]
...
models_name = [
    [f"speckle_case/normMSE_60_hc_13_ks_2_ps_16_ls_1e-06_vb"] * n_sample for v in hparam
]
text = [[f"{label}" for epoch in epochs[i]] for i, label in enumerate(labels)]
...
epochs = [[i * 1000 for i in range(n_sample)], [i * 1000 for i in range(16)]]
diff_soglia = [[1] * n_sample] * n_hc
...
text = [[f"vb={label} " for epoch in epochs[i]] for i, label in enumerate(labels)]
title = f"Gradient descent evolution"
variable_lr = [[False] * n_sample] * n_hc
early_stopping = [[False] * n_sample] * n_hc
lr = [[1] * n_sample] * n_hc
...
text = [[f"ls={ls} " for ls in xticks] for i, label in enumerate(labels)]
title = f"Gradient descent evolution"
variable_lr = [[False] * n_sample] * n_hc
early_stopping = [[False] * n_sample] * n_hc
lr = [[1] * n_sample] * n_hc
...
models_name = [
    [
        f"meyer_case/cnn_for_gaussian_test_3_60_hc_13_ks_2_ps_16_ls_0.001_vb"
        for v in xticks
    ]
] * n_hc
text = [[f"ls={ls} " for ls in xticks] for i, label in enumerate(labels)]
...
for idx in [0, -1]:
    for jdx in [-1]:
        dn_l = np.sqrt(
            np.sum((ra.recon_n[idx][jdx] - ra.min_n[idx][jdx]) ** 2, axis=1)
        ) / np.sqrt(np.sum(ra.recon_n[idx][jdx], axis=1))
...
for i in range(dn_l.shape[0]):
    plt.plot(
        [xs[j][i] for j in range(2)],
        [ys[j][i] for j in range(2)],
        linestyle="--",
        alpha=0.5,
        color="orange",
    )
...
ra.test_models_vae(
    idx,
    jdx,
    "data/dataset_meyer/dataset_meyer_test_256_100.npz",
    batch_size=100,
    plot=True,
)
...
for idx in idxs:
    for jdx in jdxs:
        ...
        plt.hist(
            eng_pred[0][0].detach().numpy() - e_target[0 : eng_pred[0][0].shape[0]],
            bins=40,
            label="pred" + label[idx],
        )
        plt.hist(
            eng_pred[0][0].detach().numpy() - eng_gd[0][0].detach().numpy(),
            bins=40,
            label="gd",
        )
        ...
        dde = np.abs(eng_pred[0][0].detach().numpy() - e_target[0:105]) - np.abs(
            eng_pred[0][0].detach().numpy() - eng_gd[0][0].detach().numpy()
        )
...
print(
    np.arange(105)[
        (eng_pred[0][0].detach().numpy() - eng_gd[0][0].detach().numpy()) < -0.002
    ]
)
...
for index in range(10):
    ...
    x_init = (
        ra.decoding(idx=[0], jdx=[0], z=ra.z_min[idx[0]][jdx[0]])[0].detach().numpy()
    )
    x_fin = ra.decoding(idx=[0], jdx=[0], z=ra.z_gs[idx[0]][jdx[0]])[0].detach().numpy()
    x_init_2 = (
        ra.decoding(idx=[0], jdx=[0], z=ra.z_min[idx[1]][jdx[0]])[0].detach().numpy()
    )
    plt.plot(
        x_init[index],
        label=f"init 1 index={index}",
...
f_trapz = ra.min_eng[idx][jdx] - np.trapz(
    v[0 : ra.min_eng[idx][jdx].shape[0]] * ra.min_n[idx][jdx], dx=ra.dx
)
f_sr = (
    ra.min_eng[idx][jdx]
    - np.sum(v[0 : ra.min_eng[idx][jdx].shape[0]] * ra.min_n[idx][jdx], axis=1) * ra.dx
)
...
data_ising = np.load(
    "data/ising_dataset/valid_sequential_periodic_64_l_0.5_h_0.5_delta_15000_n.npz"
)
...
cnn_ising = (
    cnn_ising - np.average(n, axis=0)[None, :] * np.average(n, axis=0)[:, None]
) / np.einsum("ia,ia->a", n, n)[None, :]
...
cnn_speckle = (
    cnn_speckle
    - np.average(n_speckle, axis=0)[None, :] * np.average(n_speckle, axis=0)[:, None]
) / np.einsum("ia,ia->a", n_speckle, n_speckle)[None, :]
...
model = torch.load(
    "model_dft_pytorch/isingVAE_h_2.4_20_hc_5_ks_2_ps_16_ls_0.0001_vb",
    map_location="cpu",
)
...
np.savez(
    "data/dataset_speckle_3d/train.npz",
    energy=e_train,
    F=f_train,
    density=n_train,
    potential=v_train,
...
np.savez(
    "data/dataset_speckle_3d/test.npz",
    energy=e_test,
    F=f_test,
    density=n_test,
    potential=v_test,
